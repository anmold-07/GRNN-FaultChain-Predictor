...
class QLearningSolverClass(object):
    ...
    def transition_extension_q_value(self, actionMask, current_round, actionDecision, currentState):
        ...
        if self.selectCase=="39":
            with open("39bus_priorQlearningTable_for_06", "rb") as myFile:
                """
                """
            ...
            with open("118bus_priorQlearningTable_for_10", "rb") as myFile:
                """
                """
                # actionSpace to state mapping which is of length self.NUM_STATES!
                priorQlearningTable = pickle.load(myFile)
...
if __name__ == '__main__':
    ...
    M = 5                                  # M is with respect to the 1% threhsold for risky FC determination in percentage
    EPSILON_1_m = 0.01                     # enforcing minimum level of exploration
    GAMMA = 0.99

    storeQtable = False 
    M_episodes_qlearning = 50            # set to high 5000 when storing Q-table else set to low value.
    ...
    #KaiSunAlgorithm.riskyFaultChainDict_all = riskyFaultChainDict_all
    ...
    if iteration_track:
        ...
        EPSILON_20 = 1.3
        KaiSunAlgorithm.transition_extension_run_epoch(EPSILON_20)
        end = time.time()
        print("Total Run Time: ", end - start)
        #if storeQtable:
        #    with open("118bus_priorQlearningTable_for_10", "wb") as fp:
        #        pickle.dump(KaiSunAlgorithm.q_func, fp)
        KaiSunAlgorithm.plot_rewards()
        KaiSunAlgorithm.plot_num_risky_fcs()
        KaiSunAlgorithm.plot_fc_risk()
        KaiSunAlgorithm.plot_epsilon_decay()
        print("Q-learning: Found ",  KaiSunAlgorithm.how_many_risky_fcs(), " Risky FCs in ", KaiSunAlgorithm.M_episodes, " search trials")
        #print("Q-learning: Found ",  sum( KaiSunAlgorithm.numRiskyFaultChain_5percent  ), "  5% Risky FCs in ", KaiSunAlgorithm.M_episodes, " search trials")
        #print("Q-learning: Found ",  sum( KaiSunAlgorithm.numRiskyFaultChain_10percent ), " 10% Risky FCs in ", KaiSunAlgorithm.M_episodes, " search trials")
        #print("Q-learning: Found ",  sum( KaiSunAlgorithm.numRiskyFaultChain_15percent ), " 15% Risky FCs in ", KaiSunAlgorithm.M_episodes, " search trials")
        print("Q-learning: The cumulative risk is ", sum( KaiSunAlgorithm.rewards ))
        print("Q-learning: Number of ALL False conditions: ", KaiSunAlgorithm.allFalseCount)

    else:
        start = time.time()
        #KaiSunAlgorithm.run_epoch_time()
        EPSILON_20 = 1.3
        KaiSunAlgorithm.transition_extension_run_epoch_time(EPSILON_20)
        end = time.time()
        print("Total Run Time: ", end - start)
        KaiSunAlgorithm.plot_rewards_time()
        KaiSunAlgorithm.plot_fc_risk_time()
        KaiSunAlgorithm.plot_num_risky_fcs_time()
        print("Q-learning: Found ",  KaiSunAlgorithm.how_many_risky_fcs_time(), " Risky FCs in ", KaiSunAlgorithm.i_episode , " search trials")
        print("Q-learning: The cumulative risk is ", sum( [episode[0] for episode in KaiSunAlgorithm.rewards_time] ))
        print("Q-learning: Number of ALL False conditions: ", KaiSunAlgorithm.allFalseCount)