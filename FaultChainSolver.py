...
class FaultChainSovlerClass(object):
    def __init__(self, LOADING_FACTOR, selectCase):
    ...
    def environmentStep(self, actionSpace):
        """
        Make sure to take care of off-by-one errors compare to MATPOWER!
        """
        ...
        if self.selectCase == "14":
            ...
            ppc = case118()  # looking at only the 50 most heavy loaded components
    ...
    def load_dataset(self, dataSetString):
        """
        This function loads the appropriate dataset and sets the 1% threhsold required to print dataset information.
        """
        ...
        with h5py.File(dataSetString, 'r') as hf:
            # Use os.path.basename to get just the filename without the path
            key = os.path.basename(dataSetString)[:-3]
            dataSet = hf[key][:]

        return dataSet

    def print_datatset_information(self, dataSet):
        """
        Prints useful dataset information to summarize the ground truth fault chains.
        """
        print("number of FCs:                            ", dataSet.shape[0])
        print("number of fault FCs (1% load shed rule):  ", np.sum(dataSet[:, 3] > self.threshold*1))   
        print("number of fault FCs (5% load shed rule):  ", np.sum(dataSet[:, 3] > self.threshold*5))
        print("number of fault FCs (10% load shed rule): ", np.sum(dataSet[:, 3] > self.threshold*10))
        print("number of fault FCs (15% load shed rule): ", np.sum(dataSet[:, 3] > self.threshold*15))
        print("number of fault FCs (20% load shed rule): ", np.sum(dataSet[:, 3] > self.threshold*20))
        print("maximum load shed among all FCs:          ", max(dataSet[:, 3]))
    ...
    def print_current_answer_details(self, currentAnswer):
        ...
        """
        #print("Adjacenecy Matrix: ", currentAnswer[0])
        #print("Voltage Angles: ", currentAnswer[1])
        print("Load Shed: ", currentAnswer[2])
        print("is Terminal? ", currentAnswer[3])
        print("Total Load Shed: ", currentAnswer[4])
        #print("Action Decision: ", currentAnswer[5])
...
if __name__ == '__main__':
    ...
    LOADING_FACTOR = 0.6
    FCsovler = FaultChainSovlerClass(LOADING_FACTOR, selectCase) # given snapshot (given loading condition) of the power-grid, you can use this FCsovler
    ...
    # just to check if the solver works!
    actionSpace = [8, 6, 7]
    currentAnswer = FCsovler.environmentStep(actionSpace)
    FCsovler.print_current_answer_details(currentAnswer)

    # to quantify the performance explicitly!
    
    dataSet = FCsovler.load_dataset(dataset_path)
    FCsovler.print_datatset_information(dataSet)
    M = 5                       # risky fault-chain threshold
    riskyFaultChainDict = FCsovler.find_risky_fault_chains(dataSet, M)
    print(f"length of riskyFaultChainDict: {len(riskyFaultChainDict)}")

    riskyFaultChainDict_all = FCsovler.find_risky_fault_chains_all(dataSet)
    print(f"length of riskyFaultChainDict to 5, 10, 15 %threshold: {[len(riskyFaultChainDict) for riskyFaultChainDict in riskyFaultChainDict_all]}")