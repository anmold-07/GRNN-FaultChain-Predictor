...
class DGRQNSolverClass(object):
    ...
    def train_grqn_already_experience(self):      
        ...
        if self.selectCase == "39":
            ...
            with open("state_dict_for_IEEE_118.txt", "rb") as myFile:
                """ 
                stateDict          : <class 'dict'>
                stateDict.keys()   : tuples of sequences
...
if __name__ == '__main__':
    ...
    NUM_ACTIONS = 50           # 46 for 39-bus system, 50 for 118-bus system

    LOADING_FACTOR = 0.6     # (0.55, 0.6) for the 39-bus system and (0.6, 1.0) for the 118-bus system
    dataSetString = "118bus_loading06.h5" # which data set to load for evaluation?
                                                    # 118 bus system: 118bus_loading06.h5, 118bus_loading10.h5  
                                                    #  39 bus system: loading055_39bus.h5,  loading06_39bus.h5
    ...
    EXPLORE = 250
    M_episodes_drqn = 50      # not relavant for time based solutions!
    ...
    dimInputSignals = 1
    dimOutputSignals = 48               # 12 for 39-bus system, 24 for 118-bus system
    dimHiddenSignals = 48               # 12 for 39-bus system, 24 for 118-bus system
    nFilterTaps = [3, 3]                # [3, 3] for 39-bus system, [5, 5] for 118-bus system
    bias = True
    ...
    eps = 1.00                  # no point starting below this since when filling the experience buffer
    replay_buffer_size = 5000
    sample_length = 3
    batch_size = 32
    learning_rate = 0.0005      # 39bus: 0.005, 118bus:  make sure this changes over time!

    ProposedAlgorithm = DGRQNSolverClass(M_episodes_drqn, NUM_ACTIONS, LOADING_FACTOR, selectCase, dataSetString, M, EPSILON_1_m, GAMMA, numNodes, EXPLORE, replay_buffer_size, sample_length, batch_size, learning_rate, eps, dimInputSignals, dimOutputSignals, dimHiddenSignals, nFilterTaps, bias, nonlinearityHidden, nonlinearityOutput, nonlinearityReadout, dimReadout, dimEdgeFeatures, 3)    
    ProposedAlgorithm.riskyFaultChainDict = riskyFaultChainDict
    #ProposedAlgorithm.riskyFaultChainDict_all = riskyFaultChainDict_all

    fill_Buffer = False   # to determine if you're offline or online. 
    iteration_track = 1

    if fill_Buffer:
        # this step is supposed to be performed off-line!
        ProposedAlgorithm.fill_experience_buffer()
        # in order to store the relevant variables offline!
        if selectCase =="39":
            with open("39bus_055_OfflineSequentialBuffer250", "wb") as fp:
                pickle.dump((ProposedAlgorithm.replay_buffer.counter, ProposedAlgorithm.replay_buffer.storage), fp)
        elif selectCase =="118":
            # 118bus_06_OfflineSequentialBuffer450 or 118bus_06_OfflineSequentialBuffer250
            with open("118bus_06_OfflineSequentialBuffer250", "wb") as fp:
                pickle.dump((ProposedAlgorithm.replay_buffer.counter, ProposedAlgorithm.replay_buffer.storage), fp)
        
        """
        # this step is supposed to be performed off-line!
        ProposedAlgorithm.numriskyfcs_fill_experience_buffer()
        # in order to store the relevant variables offline!
        if selectCase =="39":
            with open("06_numriskyfcs_OfflineSequentialBuffer32", "wb") as fp:
                pickle.dump((ProposedAlgorithm.replay_buffer.counter, ProposedAlgorithm.replay_buffer.storage), fp)
        elif selectCase =="118":
            with open("118bus_10_OfflineSequentialBuffer250", "wb") as fp:
                pickle.dump((ProposedAlgorithm.replay_buffer.counter, ProposedAlgorithm.replay_buffer.storage), fp)
        """
    else:
        if selectCase =="39":
            with open("39bus_055_OfflineSequentialBuffer250", "rb") as fp:
                a, b = pickle.load(fp)
        elif selectCase =="118":
            print("250")
            # 118bus_06_OfflineSequentialBuffer250 or 118bus_06_OfflineSequentialBuffer450
            with open("118bus_06_OfflineSequentialBuffer250", "rb") as fp:
                a, b = pickle.load(fp)

        ProposedAlgorithm.replay_buffer.counter = a 
        ProposedAlgorithm.replay_buffer.storage = b

        #print(ProposedAlgorithm.replay_buffer.counter)
        #print([episode[2][3] for episode in ProposedAlgorithm.replay_buffer.storage[:ProposedAlgorithm.replay_buffer.counter]])
        ...
        if iteration_track:
            ...
            print("time taken: ", end - start)
            #EPSILON_20 = 1.3
            #ProposedAlgorithm.transition_extension_train_grqn_already_experience(EPSILON_20) # this idea did not work out.
            ...
            #print("GRQN: Found ", sum( ProposedAlgorithm.numRiskyFaultChain_5percent  ), "  5% Risky FCs in ", ProposedAlgorithm.M_episodes, " search trials")
            #print("GRQN: Found ", sum( ProposedAlgorithm.numRiskyFaultChain_10percent ), " 10% Risky FCs in ", ProposedAlgorithm.M_episodes, " search trials")
            #print("GRQN: Found ", sum( ProposedAlgorithm.numRiskyFaultChain_15percent ), " 15% Risky FCs in ", ProposedAlgorithm.M_episodes, " search trials")
            ...
            start = time.time()
            ProposedAlgorithm.train_grqn_already_experience_time()
            end = time.time()
        ...
        print(" ")
        #ProposedAlgorithm.transition_extension()
