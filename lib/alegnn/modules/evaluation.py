...
def evaluate(model, data, **kwargs):
    ...
    # already converted torch.float64 datatype
    ...
    with torch.no_grad():
        ...
        splitTuple = torch.split(xTest, 2000, dim=0)
        solution = torch.empty((0), device = device)
        for b in splitTuple:
            temp = model.archit(b)
            solution = torch.cat([solution, temp])
...
def evaluateInstabilityGRNN(model, data, thresholds, window, **kwargs):
    ########
    # DATA #
    ########
    device = model.device
    ...
    # testMeasuredProfile in torch
    # testProfileHelper in numpy
    testMeasuredProfile, testProfileHelper = data.getTestProfiles('test')
    testMeasuredProfile = testMeasuredProfile.to(device)

    ########################
    #  Testset Information #
    ########################
    totalTimeProfile = testMeasuredProfile.shape[1]        
    totalSamplesProfile = (totalTimeProfile - window + 1) 
    numProfiles = testMeasuredProfile.shape[0]
    results = {}
    metrics = np.zeros([len(thresholds), numProfiles, 3])

    for profile in range(testMeasuredProfile.shape[0]):

        xTest = torch.empty([totalSamplesProfile, 20, 3, nodes], device = model.device)
        profileInfo = testProfileHelper[profile, :]
        truth = profileInfo[2]
        decision = -1
        delay = None
        metrics[:, profile, 2] = delay

        # breaking the profile in windows
        for startWindow in range(totalSamplesProfile):
            currWindow = testMeasuredProfile[profile, startWindow:startWindow + window, :3, :]
            xTest[startWindow, :, :, :] = currWindow
        ...
        # Process the samples
        with torch.no_grad():
            splitTuple = torch.split(xTest, 1000, dim=0)
            solution = torch.empty((0), device = device)
            for b in splitTuple:
                temp = model.archit(b)
                solution = torch.cat([solution, temp])
            ...
            # yHatTest is of shape
            # dimension : totalSamplesProfile x numberOfClasses
            
            m = nn.Softmax(dim=1)
            yHat = m(yHatTest)
            # yHat is of shape (normalized probability)
            # dimension : totalSamplesProfile x numberOfClasses
            ...
            """
            matches = torch.argmax(yHat, dim=1)
            matches = (matches == 4).nonzero()
            if matches.size(0)==0:
                decision = -1
            else:
                decision = matches[0].item()
            """

            for index in range(len(thresholds)):

                matches = (yHat[:,4] > thresholds[index] ).nonzero()
                if matches.size(0)==0:
                    decision = -1
                else:
                    decision = matches[0].item()
                ...
                if truth!= -1 and decision!= -1:
                    delay = truth - decision
                    metrics[index, profile, 2] = delay

                elif truth!= -1 and decision== -1: 
                    # un-stable profile but decision stable 
                    # Missed Detection (MD)
                    metrics[index, profile, 1] = 1

                elif truth== -1 and decision!= -1: 
                    # stable profile but decision unstable
                    # False Alarm (FA)
                    metrics[index, profile, 0] = 1  


    for index in range(len(thresholds)):

        falseAlarm = np.sum(metrics[index, :, 0])
        missedDetection = np.sum(metrics[index, :, 1])

        boool = np.isnan(metrics[index, :, 2])
        indices = np.invert(boool).nonzero()[0]
        averageDelay = np.average(metrics[index, indices, 2])
        results[thresholds[index]] = (falseAlarm, missedDetection, averageDelay)
