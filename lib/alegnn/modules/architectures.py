...
class GraphRecurrentNNclassification(nn.Module):
# Anmol Dwivedi, dwivea2@rpi.edu
    ...
    def __init__(self,
        ...
        if len(self.dimReadout) > 0: # Maybe we don't want to readout anything
            ...
            # filtering stage to create the number of features required by
            # the readout layer
            fc.append(nn.Linear(self.G*self.N, dimReadout[0], bias = self.bias))
            # The last linear layer cannot be followed by nonlinearity, because
            # usually, this nonlinearity depends on the loss function (for
            # instance, if we have a classification problem, this nonlinearity
            # is already handled by the cross entropy loss or we add a softmax.)
    ...
    def splitForward(self, x):
        ...
        yOut = self.outputState(z)
        yOut = self.rho(yOut) # Don't forget the nonlinearity!
        yOut = yOut.reshape((B,self.G*N))
        # B x G*N
        y = self.Readout(yOut)
        return y, yOut
        # B x dimReadout[-1], B x G*N
...
class VanillaRecurrentNNclassification(nn.Module):
# Anmol Dwivedi, dwivea2@rpi.edu
    ...
    def __init__(self,
        ...
        self.hiddenSize = dimHiddenSignals
        self.numLayers = numLayers
        self.bias = bias
        self.rnn = nn.RNN(dimInputSignals, dimHiddenSignals, numLayers, batch_first=True)
        self.fc = nn.Linear(dimHiddenSignals, dimReadout[0])

    def splitForward(self, x):
        # Check the dimensions of the input
        #   x: B x T x F[0] x N
        # the issue is that x is of the above shape. 
        # Hence, x has to be reshaped for proper traning;

        x = x.reshape(x.shape[0], x.shape[1], -1)

        h0 = torch.randn((self.numLayers, x.size(0), self.hiddenSize), device = x.device)  
        out, _ = self.rnn(x, h0)
