...
class Trainer:
    ...
    def trainBatch(self, thisBatchIndices):
        ...
        # weights = torch.tensor([1, 519354/4060, 519354/7890, 519354/4060, 519354/110379])
        # loss = self.model.loss(weight=weights)
        loss = self.model.loss()
        lossValueTrain = loss(yHatTrain, yTrain)
        ...
        # Compute the accuracy (Cost Function)
        ...
        # In case you want to know which ones are correct and which ones are wrong, 
        # drill down on the data.evaluate method.
    ...
    def validationStep(self):
        ...
        with torch.no_grad():
            ...
            # Obtain the output of the GNN (issue for large xValid. Need a way to break it up)
            splitTuple = torch.split(xValid, 2000, dim=0)
            solution = torch.empty((0), device = self.model.device)
            for b in splitTuple:
                temp = self.model.archit(b)
            ...
            #weights = torch.tensor([1, 58460/440, 58460/760, 58460/440, 58460/9882])
            #loss = self.model.loss(weight=weights)
            loss = self.model.loss()
