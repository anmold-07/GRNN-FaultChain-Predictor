...
class _data:
    ...
    def astype(self, dataType):
        ...
        # Now that we have selected the dataType, and the corresponding labelType, 
        # we can proceed to convert the data into the corresponding type
        for key in self.samples.keys():
        ...
        self.profiles['test']['signals'] = changeDataType(
                                                   self.profiles['test']['signals'],
                                                   dataType)
...
class TransientStability3m9b(_dataForClassification):
# Anmol Dwivedi, dwivea2@rpi.edu.
    """
    Responsible for creating "training", "validation" and "test" 
    datasets from the given profiles dataset "dataSet.mat".
    """
    def __init__(self, threshold, nTrain, nValid, nTest, dataType = np.float64, device = 'cpu'):
        super().__init__()
        self.profiles = {}
        self.profiles['test'] = {}
        self.profiles['test']['signals'] = None
        self.profiles['test']['helper'] = None
        self.dataType = dataType
        self.device = device
        self.Adj = self.createGraph()
        self.N = self.Adj.shape[0]
        ...
        # loading the dataset on local machine
        saveDirRoot = 'Mac 2500 Profiles'
        thisFilename = 'dataSet.mat'
        dataSet = os.path.join(saveDirRoot, thisFilename)
        dataSetPath = os.path.join(os.getcwd(), dataSet)
        dataSetProfile = scipy.io.loadmat(dataSetPath)
        
        # consider only a subset of the dataset (first 1000 for now)
        dataSetProfile["measuredTensor"] = dataSetProfile["measuredTensor"][:1000, :, :, :]
        dataSetProfile["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][:1000, :]

        # Data pre-processing (constrain the absolute value important)
        dataSetProfile["measuredTensor"][abs(dataSetProfile["measuredTensor"]) < threshold] = 0

        # Data splitting window logic
        UnStableProfiles = dataSetProfile["profileHelperTensor"][:,2]!=-1
        StableProfiles = dataSetProfile["profileHelperTensor"][:,2]==-1
        UnStableProfilesIndices = np.where(UnStableProfiles==True)[0]
        StableProfilesIndices = np.where(StableProfiles==True)[0]

        # Data splitting window logic

        # for 250
        """
        dataSetProfileTrain = {"measuredTensor":np.empty([160, 3200, 5, 3]), "profileHelperTensor":np.empty([160, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[18:], StableProfilesIndices[72:]), axis=0)
        dataSetProfileTrain["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"measuredTensor":np.empty([40, 3200, 5, 3]), "profileHelperTensor":np.empty([40, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[10:18], StableProfilesIndices[40:72]), axis=0)
        dataSetProfileValidation["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"measuredTensor":np.empty([50, 3200, 5, 3]), "profileHelperTensor":np.empty([50, 8])}
        testIndices = np.concatenate((UnStableProfilesIndices[:10], StableProfilesIndices[:40]), axis=0)
        dataSetProfileTest["measuredTensor"] = dataSetProfile["measuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][testIndices, :]
        """
        
        # for 1000
        
        dataSetProfileTrain = {"measuredTensor":np.empty([640, 3200, 5, 3]), "profileHelperTensor":np.empty([640, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[40+32:], StableProfilesIndices[160+128:]), axis=0)
        dataSetProfileTrain["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"measuredTensor":np.empty([160, 3200, 5, 3]), "profileHelperTensor":np.empty([160, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[40:40+32], StableProfilesIndices[160:160+128]), axis=0)
        dataSetProfileValidation["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"measuredTensor":np.empty([200, 3200, 5, 3]), "profileHelperTensor":np.empty([200, 8])}
        testIndices = np.concatenate((UnStableProfilesIndices[:40], StableProfilesIndices[:160]), axis=0)
        dataSetProfileTest["measuredTensor"] = dataSetProfile["measuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][testIndices, :]
        ...
        # for 2500
        """
        dataSetProfileTrain = {"measuredTensor":np.empty([1800, 3200, 5, 3]), "profileHelperTensor":np.empty([1800, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[141:], StableProfilesIndices[561:]), axis=0)
        dataSetProfileTrain["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"measuredTensor":np.empty([200, 3200, 5, 3]), "profileHelperTensor":np.empty([200, 8])}
        trainIndices = np.concatenate((UnStableProfilesIndices[100:141], StableProfilesIndices[400:561]), axis=0)
        dataSetProfileValidation["measuredTensor"] = dataSetProfile["measuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"measuredTensor":np.empty([500, 3200, 5, 3]), "profileHelperTensor":np.empty([500, 8])}
        testIndices = np.concatenate((UnStableProfilesIndices[:100], StableProfilesIndices[:400]), axis=0)
        dataSetProfileTest["measuredTensor"] = dataSetProfile["measuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["profileHelperTensor"] = dataSetProfile["profileHelperTensor"][testIndices, :]
        """        

        stepSize = 0.005   # transient stability 5 ms simulation
        samplingFreq = 60  
        window = int((6/samplingFreq)*(1/stepSize))

        # cutting the training profiles into windows
        totalTimeinProfile = dataSetProfileTrain["measuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileTrain["measuredTensor"].shape[0])
        count, Xtrain, Ytrain = 0, np.empty([totalSamples, 20, 3, 3]), np.zeros([totalSamples, 1])

        for profile in range(dataSetProfileTrain["measuredTensor"].shape[0]):

            for startWindow in range(totalTimeinProfile-window+1):

                currProfile = dataSetProfileTrain["measuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xtrain[count, :, :, :] = currProfile
                Ytrain[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileTrain["profileHelperTensor"][profile, :])
                count += 1
        Ytrain = Ytrain.squeeze(1)
        Ytrain = torch.tensor(Ytrain, dtype=torch.long) 

        # cutting the validation profiles into windows
        totalTimeinProfile = dataSetProfileValidation["measuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileValidation["measuredTensor"].shape[0])
        count, Xvalidation, Yvalidation = 0, np.empty([totalSamples, 20, 3, 3]), np.zeros([totalSamples, 1])
        for profile in range(dataSetProfileValidation["measuredTensor"].shape[0]):
            for startWindow in range(totalTimeinProfile-window+1):
                currProfile = dataSetProfileValidation["measuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xvalidation[count, :, :, :] = currProfile
                Yvalidation[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileValidation["profileHelperTensor"][profile, :])
                count += 1        
        Yvalidation = Yvalidation.squeeze(1)
        Yvalidation = torch.tensor(Yvalidation, dtype=torch.long) 

        # cutting the test profile into windows; but this is not actually needed; 
        # rather we need to process the entire test profile from the learned model;
        totalTimeinProfile = dataSetProfileTest["measuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileTest["measuredTensor"].shape[0])
        count, Xtest, Ytest = 0, np.empty([totalSamples, 20, 3, 3]), np.zeros([totalSamples, 1])

        for profile in range(dataSetProfileTest["measuredTensor"].shape[0]):
            
            for startWindow in range(totalTimeinProfile-window+1):
                
                currProfile = dataSetProfileTest["measuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xtest[count, :, :, :] = currProfile
                Ytest[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileTest["profileHelperTensor"][profile, :])
                count += 1
        Ytest = Ytest.squeeze(1)
        Ytest = torch.tensor(Ytest, dtype=torch.long) 

        # to be trained & validated on windows
        self.samples['train']['signals'] = Xtrain
        self.samples['train']['targets'] = Ytrain
        self.samples['valid']['signals'] = Xvalidation
        self.samples['valid']['targets'] = Yvalidation
        self.samples['test']['signals'] = Xtest
        self.samples['test']['targets'] = Ytest

        # to be tested on entire profiles (modify the necessary)
        self.profiles['test']['signals'] = dataSetProfileTest["measuredTensor"][:,:,:3,:]
        self.profiles['test']['helper'] = dataSetProfileTest["profileHelperTensor"]

        self.astype(self.dataType)
        self.to(self.device)

    def doLabelWindow(self, windowStartTime, windowEndTime, windowHelper):
        Y = -1
        startFaultTime = windowHelper[0]
        clearFaultTime = windowHelper[1]
        
        if windowEndTime < startFaultTime:
            Y = 0
        elif windowStartTime <= startFaultTime and windowEndTime >= startFaultTime:
            Y = 1
        elif windowStartTime > startFaultTime and windowEndTime < clearFaultTime:
            Y = 2
        elif windowStartTime <= clearFaultTime and windowEndTime >= clearFaultTime: 
            Y = 3
        elif windowStartTime > clearFaultTime and windowHelper[2] == -1:
            Y = 0
        elif windowStartTime > clearFaultTime and windowHelper[2] != -1:
            Y = 4
        return Y
    ...
    def createGraph():
        saveDirRoot = 'Mac 2500 Profiles'
        thisFilename = 'GSO.mat'
        dataSet = os.path.join(saveDirRoot, thisFilename)
        dataSetGSO = os.path.join(os.getcwd(), dataSet)
        graphData = scipy.io.loadmat(dataSetGSO)

        lap = 0 - graphData["GSO"]
        np.fill_diagonal(lap, 0)
        return lap

    def getTestProfiles(self, samplesType, *args):  
        x = self.profiles[samplesType]['signals']
        y = self.profiles[samplesType]['helper']
        return x, y

    def astype(self, dataType):
        self.Adj= changeDataType(self.Adj, dataType)
        super().astype(dataType)
    ...
    # No need since this class inherits from data_classification class & that has a evaluate method;
    # def evaluate(self, yHat, y):
...
class TransientStability16m68b(_dataForClassification):
# Anmol Dwivedi, dwivea2@rpi.edu.
    """
    Responsible for creating "training", "validation" and "test" 
    datasets from the given profiles dataset "dataSet.mat".
    ...
    def __init__(self, threshold, nTrain, nValid, nTest, dataType = np.float64, device = 'cpu'):
        ...
        # loading the dataset on local machine
        saveDirRoot = 'Mac 2500 Profiles/16m68b/2'
        thisFilename = 'dataSet16m68b.mat'
        dataSet = os.path.join(saveDirRoot, thisFilename)
        dataSetPath = os.path.join(os.getcwd(), dataSet)
        dataSetProfile = scipy.io.loadmat(dataSetPath)


        dataSetProfile["checkMeasuredTensor"] = dataSetProfile['measuredTensor']
        del dataSetProfile['measuredTensor']
        dataSetProfile["checkprofileHelperTensor"] = dataSetProfile['profileHelperTensor']
        del dataSetProfile['profileHelperTensor']
        

        # consider only a subset of the dataset (first 250 for now)
        # dataSetProfile["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][:250, :, :, :]
        # dataSetProfile["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][:250, :]

        # Data pre-processing (constrain the absolute value important)
        dataSetProfile["checkMeasuredTensor"][dataSetProfile["checkMeasuredTensor"] < threshold] = 0

        # Data splitting window logic
        UnStableProfiles = dataSetProfile["checkprofileHelperTensor"][:,2]!=-1
        StableProfiles = dataSetProfile["checkprofileHelperTensor"][:,2]==-1
        UnStableProfilesIndices = np.where(UnStableProfiles==True)[0]
        StableProfilesIndices = np.where(StableProfiles==True)[0]
        ...
        # for 250
        """ 
        dataSetProfileTrain = {"checkMeasuredTensor":np.empty([160, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([160, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[25+20:], StableProfilesIndices[25+20:]), axis=0)
        dataSetProfileTrain["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"checkMeasuredTensor":np.empty([40, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([40, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[25:25+20], StableProfilesIndices[25:25+20]), axis=0)
        dataSetProfileValidation["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"checkMeasuredTensor":np.empty([50, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([50, 21])}
        testIndices = np.concatenate((UnStableProfilesIndices[:25], StableProfilesIndices[:25]), axis=0)
        dataSetProfileTest["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][testIndices, :]
        """

        # for 500
        """
        dataSetProfileTrain = {"checkMeasuredTensor":np.empty([320, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([320, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[50+40:], StableProfilesIndices[50+40:]), axis=0)
        dataSetProfileTrain["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"checkMeasuredTensor":np.empty([80, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([80, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[50:50+40], StableProfilesIndices[50:50+40]), axis=0)
        dataSetProfileValidation["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"checkMeasuredTensor":np.empty([100, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([100, 21])}
        testIndices = np.concatenate((UnStableProfilesIndices[:50], StableProfilesIndices[:50]), axis=0)
        dataSetProfileTest["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][testIndices, :] 
        """ 

        # for 1000
        dataSetProfileTrain = {"checkMeasuredTensor":np.empty([640, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([640, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[60+48:], StableProfilesIndices[140+112:]), axis=0)
        dataSetProfileTrain["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"checkMeasuredTensor":np.empty([160, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([160, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[60:60+48], StableProfilesIndices[140:140+112]), axis=0)
        dataSetProfileValidation["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"checkMeasuredTensor":np.empty([200, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([200, 21])}
        testIndices = np.concatenate((UnStableProfilesIndices[:60], StableProfilesIndices[:140]), axis=0)
        dataSetProfileTest["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][testIndices, :]

        """ 
        dataSetProfileTrain = {"checkMeasuredTensor":np.empty([640, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([640, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[100+80:], StableProfilesIndices[100+80:]), axis=0)
        dataSetProfileTrain["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileTrain["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileValidation = {"checkMeasuredTensor":np.empty([160, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([160, 21])}
        trainIndices = np.concatenate((UnStableProfilesIndices[100:100+80], StableProfilesIndices[100:100+80]), axis=0)
        dataSetProfileValidation["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][trainIndices, :, :, :]
        dataSetProfileValidation["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][trainIndices, :]

        dataSetProfileTest = {"checkMeasuredTensor":np.empty([200, 3200, 5, 16]), "checkprofileHelperTensor":np.empty([200, 21])}
        testIndices = np.concatenate((UnStableProfilesIndices[:100], StableProfilesIndices[:100]), axis=0)
        dataSetProfileTest["checkMeasuredTensor"] = dataSetProfile["checkMeasuredTensor"][testIndices, :, :, :]
        dataSetProfileTest["checkprofileHelperTensor"] = dataSetProfile["checkprofileHelperTensor"][testIndices, :]
        """
        ...
        # cutting the training profiles into windows
        totalTimeinProfile = dataSetProfileTrain["checkMeasuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileTrain["checkMeasuredTensor"].shape[0])
        count, Xtrain, Ytrain = 0, np.empty([totalSamples, 20, 3, 16]), np.zeros([totalSamples, 1])
        for profile in range(dataSetProfileTrain["checkMeasuredTensor"].shape[0]):
            for startWindow in range(totalTimeinProfile-window+1):
                currProfile = dataSetProfileTrain["checkMeasuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xtrain[count, :, :, :] = currProfile
                Ytrain[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileTrain["checkprofileHelperTensor"][profile, :])
                count += 1
        Ytrain = Ytrain.squeeze(1)
        Ytrain = torch.tensor(Ytrain, dtype=torch.long) 

        # cutting the validation profiles into windows
        totalTimeinProfile = dataSetProfileValidation["checkMeasuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileValidation["checkMeasuredTensor"].shape[0])
        count, Xvalidation, Yvalidation = 0, np.empty([totalSamples, 20, 3, 16]), np.zeros([totalSamples, 1])
        for profile in range(dataSetProfileValidation["checkMeasuredTensor"].shape[0]):
            for startWindow in range(totalTimeinProfile-window+1):
                currProfile = dataSetProfileValidation["checkMeasuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xvalidation[count, :, :, :] = currProfile
                Yvalidation[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileValidation["checkprofileHelperTensor"][profile, :])
                count += 1        
        Yvalidation = Yvalidation.squeeze(1)
        Yvalidation = torch.tensor(Yvalidation, dtype=torch.long) 
        ...
        # cutting the test profile into windows; but this is not actually needed; 
        # rather we need to process the entire test profile from the learned model;
        totalTimeinProfile = dataSetProfileTest["checkMeasuredTensor"].shape[1]
        totalSamples = (totalTimeinProfile - window + 1)*(dataSetProfileTest["checkMeasuredTensor"].shape[0])
        count, Xtest, Ytest = 0, np.empty([totalSamples, 20, 3, 16]), np.zeros([totalSamples, 1])

        for profile in range(dataSetProfileTest["checkMeasuredTensor"].shape[0]):
            
            for startWindow in range(totalTimeinProfile-window+1):
                
                currProfile = dataSetProfileTest["checkMeasuredTensor"][profile, startWindow:startWindow + window, :3, :]
                Xtest[count, :, :, :] = currProfile
                Ytest[count, 0] = self.doLabelWindow(startWindow, startWindow + window, dataSetProfileTest["checkprofileHelperTensor"][profile, :])
                count += 1
        Ytest = Ytest.squeeze(1)
        Ytest = torch.tensor(Ytest, dtype=torch.long) 
        ...
        # to be tested on entire profiles (modify the necessary)
        self.profiles['test']['signals'] = dataSetProfileTest["checkMeasuredTensor"][:,:,:3,:]
        self.profiles['test']['helper'] = dataSetProfileTest["checkprofileHelperTensor"]
