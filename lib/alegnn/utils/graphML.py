...
import lib.alegnn.utils.graphTools as graphTools
...
def LSIGF(h, S, x, b=None):
    ...
    for k in range(1,K):
        x = torch.matmul(x, S.to(x.device)) # B x E x G x N
    ...
    y = torch.matmul(z.permute(0, 4, 1, 2, 3).reshape([B, N, E*K*G]),
                     h.reshape([F, E*K*G]).permute(1, 0).to(x.device)).permute(0, 2, 1)
    # And permute againt to bring it from B x N x F to B x F x N.
    # Finally, add the bias
    if b is not None:
        y = y.to(h.device) + b
    return y.to(h.device)
...
def GatedGRNN(a, b, S, x, z0, sigma, q_hat = torch.ones(1), q_check = torch.ones(1),
    ...
    for t in range(1,T+1):
        ...
        if len(q_check.shape) < 5:
            ...
            # and apply it
            Bzt = this_q_check*Bzt.to(this_q_check.device)
        else:
